...
export default function VisibilityForm({
  ...
  if (!isOwner) {
    ...
    return (
      <Alert
        variant="info"
        title="Visibilité"
        description={`${current?.icon} ${current?.label} — ${current?.description}`}
      >
  ...
  return (
    <>
      <Alert variant="info" title="Visibilité">
        <form
          ref={formRef}
          action={formAction}
          className="mt-3"
        >
          ...
          {missingFields.length > 0 && (
            <Alert
              ...
              description={`Pour passer en Privé ou Public : ${missingFields.join(', ')}`}
              className="mb-3"
            />
          )}
          <SubmitButton
