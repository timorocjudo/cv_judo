...
export default function HeroZone({
  title = 'Ton palmarès mérite sa propre page.',
  ...
  // Delays are relative to whether the wordmark+bar lead the cascade, so the
  // compact (no-wordmark) variant still animates in as a tight sequence.
  const delay = {
    title: prefersReduced ? 0 : showWordmark ? 0.15 : 0,
    subtitle: prefersReduced ? 0 : showWordmark ? 0.3 : 0.15,
    cta: prefersReduced ? 0 : showWordmark ? 0.45 : 0.3,
  }
    ...
    <section className={`relative overflow-hidden bg-[#1B3A6B] ${minHeightClassName} flex items-center justify-center px-margin-mobile md:px-margin-desktop py-20`}>
      ...
      <div className="relative z-10 text-center max-w-4xl mx-auto w-full">
        ...
        <motion.h1
          ...
          transition={{ duration: dur, delay: delay.title, ease: 'easeOut' }}
        ...
        {subtitle && (
          <motion.p
            initial={{ opacity: 0, y: yVal }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: dur, delay: delay.subtitle, ease: 'easeOut' }}
        ...
        {cta && (
          <motion.div
            initial={{ opacity: 0, y: yVal }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: dur, delay: delay.cta, ease: 'easeOut' }}
          >
            {cta}
          </motion.div>
        )}
