...
// /dashboard, /dashboard/nouveau, /dashboard/parametres and /dashboard/setup
// share the plain dashboard layout, which renders its own footer directly
// (as a flex child alongside its min-h-screen column) so it stays visible
// without scrolling on short pages — avoid rendering a second one here.
function isPlainDashboardRoute(segments: string[]) {
  return segments[0] === 'dashboard' && (segments.length === 1 || RESERVED_DASHBOARD_SEGMENTS.has(segments[1]))
}
...
export default function FooterLegalConditional() {
  ...
  if (isPlainDashboardRoute(segments)) return null
